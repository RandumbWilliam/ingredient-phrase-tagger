...
const generateDataStream = (data_path, count, offset) => {
  ...
  const transformer = csv.transform((row) => {
    ...
    const displayInput = utils.cleanUnicodeFractions(row.input);
    const tokens = utils.tokenize(displayInput);
    delete row.input;
  ...
  return fs.createReadStream(data_path).pipe(parser).pipe(transformer);
};
...
const parseNumbers = (s: string) => {
  ...
  */
  ...
  const m3 = ss.match(/^\d+$/);
  if (m3) {
    return Number(parseFloat(ss).toFixed(2));
  }
  ...
  */
  ...
  for (const [key, val] of Object.entries(ingredientRow)) {
    if (isNaN(parseFloat(val as string))) {
      for (const vt of utils.tokenize(val as string)) {
        if (utils.normalizeToken(vt) === token) {
          ret.push(key.toUpperCase());
        }
      }
    } else if (decimalToken !== null) {
      const decimalVal = parseFloat(val as string);
      if (decimalVal === decimalToken) {
        ret.push(key.toUpperCase());
      }
    }
  }
